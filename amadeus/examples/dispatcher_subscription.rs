...
async fn main() -> Result<()> {
    ...
    // 创建三个分发器，订阅不同的消息类型
    println!("1. 创建分发器并设置订阅：\n");

    // 分发器1：订阅所有消息类型（默认）
    let dispatcher1 = Iceoryx2Dispatcher::new("service1")
        .with_name("全量分发器");
    println!("  - {}: 订阅所有消息类型", dispatcher1.name());
    ...
    println!("  - {}: 只订阅 'notification' 类型", dispatcher2.name());
    ...
    println!("  - {}: 订阅 'alert' 和 'warning' 类型", dispatcher3.name());
    ...
    println!("  发送消息: 类型='notification'");
    msg_mgr.handle_plugin_message(msg1).await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    println!("    → 应该被接收的分发器: 全量分发器, 通知分发器\n");
    ...
    println!("  发送消息: 类型='alert'");
    msg_mgr.handle_plugin_message(msg2).await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    println!("    → 应该被接收的分发器: 全量分发器, 多类型分发器\n");
    ...
    println!("  发送消息: 类型='other'");
    msg_mgr.handle_plugin_message(msg4).await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    println!("    → 应该被接收的分发器: 全量分发器（只有它订阅所有类型）\n");
    ...
    println!("3. 总结：");
    println!("  - 全量分发器：接收所有消息（订阅所有类型）");
    println!("  - 通知分发器：只接收 'notification' 类型");
    println!("  - 多类型分发器：只接收 'alert' 和 'warning' 类型");
